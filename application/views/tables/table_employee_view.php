...
$tmpl = array (
'table_open'          => '<table border="0" cellpadding="4" cellspacing="0" width="300px">',
...
$this->table->clear();
//отображение табл.
$this->table->set_heading('','id','Ответственный');
if (isset($array_employee))
{
	foreach($array_employee as $index=>$value)
	{
		$chk_input = array(
		'name'=>'chk_id[]',
		'value'=>$value['id'],
		'checked'=>set_checkbox('chk_id[]',$value['id'],FALSE));
		$this->table->add_row(form_checkbox($chk_input), $value['id'],$value['Family']);
	}
}
