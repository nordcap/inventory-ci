...
$tmpl = array (
'table_open'          => '<table id= "assets" border="0" cellpadding="4" cellspacing="0">',
...
$this->table->clear();
//отображение табл.
$this->table->set_heading('','Инв. №','Наименование','Цена','Амортизация','Дата', 'Категория');
if (isset($array_assets))
{
	foreach($array_assets as $index=>$value)
	{
		$chk_input = array(
		'name'=>'chk_id[]',
		'value'=>$value['id'],
		'checked'=>set_checkbox('chk_id[]',$value['id'],FALSE));
		$this->table->add_row(form_checkbox($chk_input), $value['Inv_id'], $value['Name'], $value['Price'], $value['Amortization'], $value['Time'], $value['Name_Category']);
	}
}
